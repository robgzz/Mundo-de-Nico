...
$out=Join-Path([System.IO.Path]::GetTempPath())("mundodenico-web-"+[guid]::NewGuid().ToString("N")+".json")
try{az bicep build --file $BicepFile --outfile $out;if($LASTEXITCODE -ne 0){throw "Bicep compilation failed."};Write-Host "Bicep compilation succeeded." -ForegroundColor Green}finally{Remove-Item $out -Force -ErrorAction SilentlyContinue}
